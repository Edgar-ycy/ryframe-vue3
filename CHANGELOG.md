...
- 与 Rust 1.98.0 后端协调发布 0.12.1，同步版本化 OpenAPI 和生成资产；接口与页面行为保持不变。

## [v0.12.0] - 2026-09-02

### Added

- 新增 `tenantId / subjectId / sessionEpoch` 服务端状态作用域，查询、消息、导出、个人中心和服务账号缓存统一按租户、主体和会话代次隔离。
- 新增请求级会话快照与会话取消信号；过期 Query、Mutation、401 刷新和回调不会写入新会话，也不会显示取消错误。
- 新增按 OpenAPI 领域拆分的 typed caller，明确支持 JSON、multipart、text 和 blob 响应绑定；无法唯一判断媒体类型时生成失败。
- 新增路由 catalog registry，首屏只同步加载核心、外壳和全局导出文案，其余国际化目录随路由按需加载并支持并发去重和失败重试。

### Changed

- Vue Router、Pinia 和 Element Plus 服务改为显式导入；Store、API module、feature contract 与 `shared/http` 依赖方向由静态门禁强制执行。
- 所有管理页面、确认框、导出流程和表单 Mutation 在开始时捕获会话作用域，过期完成结果不再触发提示、回调、缓存写入或失效。
- 生成 API 按 core、system、platform、monitor 和 agent 领域输出，删除旧单体 operation 入口；手写模块只保留业务命名、参数规范化和明确 adapter。
- 开发与 CI 命令统一通过 Corepack 使用 `pnpm@11.20.0`；日常 CI 收敛为 5 个业务任务，兼容性、许可证、物料清单和漏洞扫描移入扩展门禁。
- README 与架构说明只描述当前项目的使用方式、边界和 CI 状态，不再保留历史优化对照。

### Fixed

- 修复同租户用户切换、跨租户切换、权限降低、授权 epoch 变化和跨标签认证时旧查询结果、导航投影及异步副作用回写的问题。
- 修复慢 Mutation、导出确认、管理弹窗、个人资料和服务账号操作在会话切换后仍可能显示提示或刷新新主体缓存的问题。
- 修复生成 operation 的响应媒体类型选择、嵌套 i18n 目录检查和 CI 工作流上下文作用域校验。

### Validation

- 前端生产构建 5 次实测 P50 为 3.996 秒、P95 为 4.362 秒；快速检查 P95 为 9.506 秒。
- 初始 JavaScript 为 162,102 B gzip，核心 i18n 为 8,657 B gzip，core operation 为 780 B gzip；非核心 operation 和业务目录不进入初始依赖图。
- 72 个测试文件、293 项单测、5 项浏览器 smoke 和真实 API/MySQL/Redis/Chrome 流程通过；登录页、首页、岗位与租户页无新增控制台错误或警告，axe serious/critical 为零。

## [v0.11.3] - 2026-08-27

### Changed

- 收敛单向依赖、会话、导航、消息与设置架构，保持 Pinia Store 被动并由应用协调器编排副作用。
- 提升格式化、静态检查、类型检查、资源生成、契约校验与浏览器测试的反馈速度。
- 精简面向使用者的项目文档、无关协作模板和 Qodana 残留配置。

## [v0.11.2] - 2026-08-23

### Changed

- 同步基于已应用筛选的异步导出、任务下载、取消、终态单删与批删交互。
- 统一菜单、权限、状态和时间的国际化显示，并固定配套后端契约来源。

## [v0.11.1] - 2026-08-21

### Fixed

- 修复 HTTP `202` 成功包络被误判为删除失败，并以真实状态包络覆盖单删、批删与浏览器 smoke。

## [v0.11.0] - 2026-08-21

### Added

- 新增由 OpenAPI 生成的 operation descriptor、强类型访问目录与确定性契约校验。
- 新增使用最后一次已应用筛选的导出流程，以及终态任务单删与批删交互。

### Changed

- 标准 API 调用全面迁移到生成的操作描述符，菜单、权限、路由和请求类型使用同一契约来源。
- 增加前后端不可变提交配对发布门禁，并同步后端资源与版本身份。

## [v0.10.0] - 2026-08-19

### Added

- 新增套餐、Capability、租户数据放置与停写迁移的管理界面；产品变更与迁移均通过预览、确认和任务进度闭环完成。
- 新增原子租户会话上下文与功能 manifest，动态路由、菜单、按钮、标签页和业务写入状态统一由租户 Capability、RBAC 与数据状态共同控制。
...
- 修复运维总览的趋势请求先于概览请求完成时，图表容器挂载后未补绘导致活动、调度和登录操作图表空白的问题。
- 删除侧边栏中与服务端动态菜单重复的租户管理硬编码入口，平台管理仅以会话上下文菜单树为唯一来源。
- 登录页验证码按原始清晰度展示，刷新期间防止并发覆盖；输入会去除空白并自动转为大写，同时按当前登录租户请求验证码配置。
- 修复服务账号未开通时仍展示菜单与个人中心“服务委托”卡片、并发起无效请求的问题：前端统一依据会话上下文中的 `system.service_accounts` Capability 隐藏入口并阻止路由访问。
