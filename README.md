...
```bash
corepack pnpm format                  # 格式化源码和文档
corepack pnpm lint:fix                # 修复 TypeScript、Vue 和 JavaScript 风格问题
corepack pnpm lint:styles:fix         # 修复 CSS、SCSS 和 Vue 样式问题
corepack pnpm typecheck               # 检查应用与测试类型
corepack pnpm test:unit               # 运行单元与组件测试
corepack pnpm test:targeted-coverage  # 运行关键状态流程测试并生成覆盖率
corepack pnpm test:browser-smoke      # 运行本地浏览器 smoke 测试
corepack pnpm api:check               # 检查契约、派生文件和 operation 使用
corepack pnpm check:fast              # 并行运行日常快速检查
corepack pnpm check                   # 运行完整本地检查和生产构建
...
| 类型         |       优化前 |         当前 | 运行时机             |
| ------------ | -----------: | -----------: | -------------------- |
| 日常业务任务 |         8 个 |         5 个 | push 与 pull request |
| 低频深度任务 | 3 个独立任务 | 2 个合并任务 | 每周或手动触发       |

日常任务保留静态检查、单测、生产构建、浏览器 smoke 和 Windows smoke。Node 22 兼容、许可证、物料清单和漏洞扫描仍会执行，但集中到扩展 CI，避免普通提交出现大量长期跳过的检查项。
...
`corepack pnpm api:check`，再在 `src/api/modules/` 中接入对应 operation。
