...
test('规范化许可证报告并展开版本', () => {
  assert.deepEqual(normalizeLicenseReport({
...
test('允许由已批准许可证组成的 SPDX 表达式', () => {
  assert.deepEqual(evaluateLicenseReport(policy(), {
...
test('未批准许可证必须使用精确且未过期的例外', () => {
  ...
  assert.deepEqual(evaluateLicenseReport(approved, {
    'Custom-1.0': [{ name: 'legacy', versions: ['1.2.3'] }],
  }, now), [])
  assert.match(evaluateLicenseReport(policy(), {
    'Custom-1.0': [{ name: 'legacy', versions: ['1.2.3'] }],
  }, now)[0], /未允许/u)
})
...
test('过期、字段缺失和未使用例外均失败', () => {
  ...
  assert(evaluateLicenseReport(unused, {
    MIT: [{ name: 'safe', versions: ['1.0.0'] }],
  }, now).some(error => error.includes('未使用')))
})
