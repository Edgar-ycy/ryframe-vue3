...
function isSessionUser(value: unknown): value is SessionUser {
  ...
  if (
    !['email', 'id', 'nickname', 'phone', 'tenant_id', 'tenant_name', 'username'].every((key) =>
...
function isSessionCapability(value: unknown): boolean {
  if (
    !isRecord(value) ||
    !hasExactKeys(value, ['client_config', 'code', 'schema_version', 'variant'])
  )
    return false
  return (
...
function isMenuNode(value: unknown): boolean {
  ...
  if (
    ...
    !isOptionalString(value.route_key)
  )
    return false
  if (
    value.perm_code !== undefined &&
    value.perm_code !== null &&
    (!isNonEmptyString(value.perm_code) || !isPermissionCode(value.perm_code))
  )
    return false
