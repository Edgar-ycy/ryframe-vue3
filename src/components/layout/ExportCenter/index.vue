...
function notificationMessage(job: ExportJob): { message: string, type: 'success' | 'error' | 'info' } | undefined {
  if (job.status === 'succeeded') {
    return { message: t('exportCenter.notifySucceeded', { name: exportJobDisplayName(job) }), type: 'success' }
  }
  if (job.status === 'failed') {
    return { message: t('exportCenter.notifyFailed', { name: exportJobDisplayName(job) }), type: 'error' }
  }
  if (job.status === 'cancelled') {
    return { message: t('exportCenter.notifyCancelled', { name: exportJobDisplayName(job) }), type: 'info' }
  }
  return undefined
}
