...
function translatedErrorMessage(envelope: ApiResponse, fallback: string): string {
  ...
  return key ? httpTranslate(key) : envelope.message || fallback
}
...
export async function toHttpError(error: unknown): Promise<HttpError> {
  ...
  if (axios.isAxiosError(error)) {
    ...
    const payload = await responseErrorPayload(
      error.response?.data,
      error.message || httpTranslate('shell.http.requestFailed'),
    )
  ...
  return new HttpError(
    error instanceof Error ? error.message : httpTranslate('shell.http.requestFailed'),
...
function withRetryAfter(
  ...
  return `${message} ${httpTranslate('shell.http.retryAfter', { seconds: retryAfterSeconds })}`
}
...
export function parseEnvelope<T>(response: AxiosResponse<ApiResponse<T>>): ApiResponse<T> {
  ...
  if (!isApiEnvelope(envelope)) {
    const error = new HttpError(httpTranslate('shell.http.invalidResponse'), {
  ...
  if (!isSuccessfulStatus) {
    const error = new HttpError(
      translatedErrorMessage(envelope, httpTranslate('shell.http.requestFailed')),
      {
        status: response.status,
        code: envelope.code,
