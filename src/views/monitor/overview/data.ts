...
import {
  ...
  MONITOR_OVERVIEW_TRENDS_RESOURCE,
} from '../queryResources'

/** 通过租户级 Vue Query 缓存读取运维快照。 */
export async function fetchOverviewSnapshot(tenantId: string, force = false): Promise<MonitorOverview> {
...
export async function fetchOverviewTrends(
  ...
  return queryClient.fetchQuery({
    queryKey,
    queryFn: async ({ signal }) => requireOperationData(await getMonitorOverviewTrends(range, signal)),
...
export function cancelOverviewRequests(tenantId: string): Promise<void> {
  return Promise.all([
    queryClient.cancelQueries({ queryKey: tenantQueryKey(tenantId, MONITOR_OVERVIEW_RESOURCE).slice(0, 3) }),
    queryClient.cancelQueries({ queryKey: tenantQueryKey(tenantId, MONITOR_OVERVIEW_TRENDS_RESOURCE).slice(0, 3) }),
  ]).then(() => undefined)
}
