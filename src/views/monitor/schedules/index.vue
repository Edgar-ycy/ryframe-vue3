    ...
    <el-card shadow="never" class="search-card">
      <el-form :model="queryParams" inline @submit.prevent="handleSearch">
        <el-form-item :label="t('monitor.schedules.name')">
          <el-input v-model="queryParams.name" :placeholder="t('monitor.schedules.namePlaceholder')" clearable @keyup.enter="handleSearch" />
        </el-form-item>
        <el-form-item :label="t('monitor.schedules.target')">
          <el-select v-model="queryParams.handler_key" :placeholder="t('monitor.schedules.targetPlaceholder')" clearable filterable>
            <el-option v-for="target in targets" :key="target.handler_key" :label="targetName(target.handler_key)" :value="target.handler_key" />
          </el-select>
        </el-form-item>
        <el-form-item :label="t('monitor.schedules.status')">
          <el-select v-model="queryParams.enabled" :placeholder="t('monitor.schedules.statusPlaceholder')" clearable>
            <el-option :label="t('monitor.schedules.enabled')" :value="true" />
            <el-option :label="t('monitor.schedules.disabled')" :value="false" />
          </el-select>
        </el-form-item>
        <el-form-item>
          <el-button v-perm="'monitor:schedule:list'" type="primary" icon="Search" @click="handleSearch">{{ t('monitor.schedules.search') }}</el-button>
          <el-button v-perm="'monitor:schedule:list'" icon="Refresh" @click="handleReset">{{ t('monitor.schedules.reset') }}</el-button>
        </el-form-item>
      </el-form>
    </el-card>
    ...
    <el-card shadow="never" class="schedules-card">
      ...
      <div class="table-scroll">
        <el-table v-loading="loading" :data="tableData" border stripe class="schedules-table" :empty-text="t('common.noData')">
          ...
          <el-table-column prop="cron_expression" :label="t('monitor.schedules.cron')" min-width="180" show-overflow-tooltip />
          <el-table-column prop="timezone" :label="t('monitor.schedules.timezone')" min-width="150" show-overflow-tooltip />
          <el-table-column :label="t('monitor.schedules.status')" width="108" align="center">
            <template #default="{ row }">
              <el-tag :type="row.enabled ? 'success' : 'info'" size="small">{{ row.enabled ? t('monitor.schedules.enabled') : t('monitor.schedules.disabled') }}</el-tag>
            </template>
          </el-table-column>
          ...
          <el-table-column :label="t('monitor.schedules.operation')" min-width="350" fixed="right" align="center">
            <template #default="{ row }">
              <el-button v-if="hasPermission('monitor:schedule:edit')" type="primary" link icon="Edit" :loading="editingId === row.id" :disabled="hasPendingWrite" @click="openEdit(row)">{{ t('monitor.schedules.edit') }}</el-button>
              <el-button v-if="hasPermission('monitor:schedule:edit')" :type="row.enabled ? 'warning' : 'success'" link :loading="statusPendingId === row.id" :disabled="hasPendingWrite" @click="handleStatus(row, !row.enabled)">{{ row.enabled ? t('monitor.schedules.disable') : t('monitor.schedules.enable') }}</el-button>
              <el-button v-if="hasPermission('monitor:schedule:run')" type="primary" link icon="VideoPlay" :loading="runPendingId === row.id" :disabled="hasPendingWrite" @click="handleRun(row)">{{ t('monitor.schedules.run') }}</el-button>
              <el-button type="info" link icon="Clock" :disabled="hasPendingWrite" @click="openHistory(row)">{{ t('monitor.schedules.history') }}</el-button>
              <el-button v-if="hasPermission('monitor:schedule:remove')" type="danger" link icon="Delete" :loading="removePendingId === row.id" :disabled="hasPendingWrite" @click="handleRemove(row)">{{ t('monitor.schedules.delete') }}</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
