...
describe('导出任务缓存', () => {
  it('查询键同时隔离租户、用户、列表、详情和未读计数', () => {
    expect(exportJobListQueryKey('tenant-a', 'user-a'))
      .toEqual(['tenant', 'tenant-a', 'user', 'user-a', 'export-jobs'])
    expect(exportJobDetailQueryKey('tenant-a', 'user-a', 'job-1'))
      .toEqual(['tenant', 'tenant-a', 'user', 'user-a', 'export-jobs', 'job-1'])
    expect(exportJobUnreadQueryKey('tenant-a', 'user-a'))
      .toEqual(['tenant', 'tenant-a', 'user', 'user-a', 'export-jobs', 'notifications', 'unread-count'])
  })
  ...
  it('置顶任务时去重、同步详情并保持最近一百条上限', () => {
    ...
    expect(client.getQueryData(exportJobDetailQueryKey('tenant-a', 'user-a', latest.id)))
      .toEqual(latest)
  })
  ...
  it('合并任务时保留既有顺序，并将新任务放到列表顶部', () => {
    client.setQueryData(
      exportJobListQueryKey(identity.tenantId, identity.userId),
      [exportJob('job-1'), exportJob('job-2')],
    )
  ...
  it('删除任务时同时清理列表和对应详情', () => {
    ...
    expect(client.getQueryData<ExportJob[]>(exportJobListQueryKey('tenant-a', 'user-a')))
      .toEqual([retained])
    expect(client.getQueryData(exportJobDetailQueryKey('tenant-a', 'user-a', removed.id)))
      .toBeUndefined()
  })
  ...
  it('只把成功或失败且尚未阅读的通知标记为已读', () => {
    ...
    expect(list?.map(job => job.notification_read_at ?? null))
      .toEqual([readAt, readAt, null, '2026-08-19T00:00:00.000Z'])
    expect(client.getQueryData<ExportJob>(exportJobDetailQueryKey('tenant-a', 'user-a', 'job-1'))?.notification_read_at)
      .toBe(readAt)
    expect(client.getQueryData<ExportJob>(exportJobDetailQueryKey('tenant-a', 'user-a', 'job-3'))?.notification_read_at)
      .toBeUndefined()
  })
